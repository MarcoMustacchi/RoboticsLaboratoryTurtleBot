function plot_animation_vehicle(map, time_trajectory, final_des_trajectory, final_output_trajectory, filename)
    ...
    plot(final_des_trajectory.xz, final_des_trajectory.yz,'LineWidth', 1, 'Color', [0.8500 0.3250 0.0980]);
    ...
    p = plot(final_output_trajectory.xz(1),final_output_trajectory.yz(1),'b','LineWidth', 1);
    % m = scatter(final_output_trajectory(1,1),final_output_trajectory(1,2),'>','filled','b','LineWidth', 1);    
    m = plot_vehicle(final_output_trajectory.xz(1), final_output_trajectory.yz(1), final_output_trajectory.thetaz(1), 0.5);
    
    % Iterating through the length of the time array
    for k = 1:1:length(time_trajectory)-1
        % Updating the line
        p.XData = final_output_trajectory.xz(1:k);
        p.YData = final_output_trajectory.yz(1:k);
        % Updating the point
        ...
        m = plot_vehicle(final_output_trajectory.xz(k), final_output_trajectory.yz(k), final_output_trajectory.thetaz(k), 0.5);
        % Updating the title

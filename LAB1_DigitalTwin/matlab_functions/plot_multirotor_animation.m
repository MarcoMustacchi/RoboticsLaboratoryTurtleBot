function plot_multirotor_animation(x, y, z, t)
    ...
    m = plotTransforms([x(1) y(1) z(1)],[1 1 1 0],MeshFilePath='multirotor.stl',MeshColor='black');
    ax = m.Children.Children;
    ...
    for k = 1:length(t)
        ...
        p.ZData = z(1:k);
        % Updating the point
        m = plotTransforms([x(k) y(k) z(k)],[1 1 1 0],MeshFilePath='multirotor.stl',MeshColor='black');
        ax = m.Children.Children;
        % Updating the title
        title(sprintf('Trajectory\nTime: %0.2f sec', t(k)),...
        'Interpreter','Latex');

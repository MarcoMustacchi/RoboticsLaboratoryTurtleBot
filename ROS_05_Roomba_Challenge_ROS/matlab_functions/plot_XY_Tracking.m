function plot_XY_Tracking(x_des, y_des, x, y, theta, filenameXY)
    ...
    theta_adj = theta(~isnan(theta(:))); % keep only non NaN values
    ...
    triangle_i = nsidedpoly(3, 'Center', [x(1), y(1)], 'SideLength', 0.1);
    theta_i = theta_adj(1) - 90;
    ...
    triangle_f = nsidedpoly(3, 'Center', [x(end), y(end)], 'SideLength', 0.1);
    theta_f = theta_adj(end) - 90;
    ...
    legend('Desired Trajectory','Actual Trajectory','Initial Desired Position','Final Desired Position','interpreter','latex','location','northeast');

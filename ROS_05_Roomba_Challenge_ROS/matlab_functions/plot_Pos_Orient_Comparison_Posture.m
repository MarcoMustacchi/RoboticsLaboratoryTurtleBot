function plot_Pos_Orient_Comparison(t, idx_switch, x_sim, y_sim, theta_sim, x_real, y_real, theta_real, x_des, y_des, theta_des, type)
    ...
    %% Errors
    ex_sim = x_des - x_sim(1:length(x_real));
    ey_sim = y_des - y_sim(1:length(y_real));
    etheta_sim = theta_des - theta_sim(1:length(theta_real));

    ex_real = x_des - x_real;
    ey_real = y_des - y_real;
    etheta_real = theta_des - theta_real;
